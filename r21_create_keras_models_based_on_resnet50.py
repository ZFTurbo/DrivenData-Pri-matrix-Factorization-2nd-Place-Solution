...
'''
Train neural net models based on recurrent LSTM and features extracted with ResNet50 pretrained model
Note: it was my first RNN model in this task. So now I know it will be better in case is features extracted 
from larger resolution. And it will be better to use double GRU layers with larger size.
'''
...
def batch_generator_train(files, batch_size, augment=True):
    ...
    while True:
        ...
        for i in range(len(batch_files)):
            f = OUTPUT_PATH + 'resnet50_preds/' + batch_files[i] + '.pklz'
            try:
...
def ZF_full_keras_model_for_resnet_v4():
...
def train_single_classification_model(num_fold, train_files, valid_files):
    ...
    cnn_type = 'ZF_full_keras_model_for_resnet_v4'
    print('Creating and compiling model [{}]...'.format(cnn_type))
    model = ZF_full_keras_model_for_resnet_v4()
...
'''
v2 RMSprop 0.0002, Double LSTM, rdrop: 0.2, size: 224
Fold 1: 0.0389843957173 
Fold 2: 0.0386066806596
Fold 3: 0.0392783166743
Fold 4: 0.0393113807449
Fold 5: 0.0391304605454
'''