...
'''
Process test data with GRU neural network for VGG16 data and create test part for 2nd level features.
Also process non-labeled data
'''

from a00_common_functions import *
from r21_create_keras_models_based_on_VGG16 import ZF_full_keras_model_GRU_1024_VGG16_v11
import math
...
def predict_generator(files, batch_size):
    ...
    while True:
        ...
        for i in range(len(batch_files)):
            f = OUTPUT_PATH + 'vgg16_preds/' + batch_files[i] + '.pklz'
            try:
...
def process_tst(nfolds):
    ...
    cnn_type = 'ZF_full_keras_model_GRU_1024_VGG16_v11'
    ...
    for num_fold in range(1, nfolds + 1):
        ...
        if not os.path.isfile(cache_path) or recalc == 1:
            ...
            model = ZF_full_keras_model_GRU_1024_VGG16_v11()
    ...
    subm.to_csv(FEATURES_PATH + 'ZF_full_keras_model_GRU_1024_VGG16_v11_test.csv')
...
def process_other_files(nfolds):
    ...
    files_filtered = []
    for pf in files:
        f = OUTPUT_PATH + 'vgg16_preds/' + pf + '.pklz'
        try:
            arr = load_from_file(f)
            if arr.shape[0] < 112 or arr.shape[1] != 512:
    ...
    file_path = FEATURES_PATH + 'ZF_full_keras_model_GRU_1024_VGG16_v11_other.csv'
